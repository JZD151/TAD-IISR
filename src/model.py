...
# ------------------------------
# VAE skip-connection hooks
# ------------------------------
# These two functions mirror the pix2pix_turbo idea: cache encoder activations
# and inject them into the decoder via lightweight 1x1 conv projections.
...
def my_vae_decoder_fwd(self, sample: torch.Tensor, latent_embeds=None) -> torch.Tensor:
    ...
    ignore_skip = getattr(self, "ignore_skip", False)
    gamma = getattr(self, "gamma", 1.0)
    if not ignore_skip:
        skip_convs = [self.skip_conv_1, self.skip_conv_2, self.skip_conv_3, self.skip_conv_4]
        incoming_skip_acts = getattr(self, "incoming_skip_acts", None)
        if incoming_skip_acts is None:
            raise AttributeError("decoder.incoming_skip_acts is not set; did you run the encoder first?")

        # up
        for idx, up_block in enumerate(self.up_blocks):
            skip_in = skip_convs[idx](incoming_skip_acts[::-1][idx] * gamma)
